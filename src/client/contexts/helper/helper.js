...
async function updateListingsByFilters(mapElementID, listings, filterParams) {
    ...
    if (search.length > 0) {
        ...
        if (geometry.status === "OK") {
            const location = geometry.results[0]["geometry"]["location"];
            const viewport = geometry.results[0]["geometry"]["viewport"];
            ...
            if (document.getElementById(mapElementID)) { // Continue if element exists.
                ...
                const zoom = getBoundsZoomLevel(viewport, {height: mapViewProperties.height, width: mapViewProperties.width});
                const boundParams = {bounds: viewport, center: location, zoom: zoom};
                ...
                for (let listing of listings) {
                    if (filterParams["places"][listing.rental_property_information.room_type]) {
                        ...
                        if ((askingPrice >= min && askingPrice <= max) || askingPrice > 1000) {
                            const bounds = boundParams["bounds"];
                            if (bounds) {
                                if (isInsideBounds(listing.rental_property_information.coordinates, bounds.northeast, bounds.southwest)) {
                                    filteredListings.push(listing);
                                }
                ...
                return [filteredListings, boundParams];
            }
        }
    }
