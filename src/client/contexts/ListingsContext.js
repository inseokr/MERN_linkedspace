...
export function ListingsProvider(props) {
  ...
  function filterListings(places, price, date) {
    const newFilteredListings = [];
    for (const listing of listingsByBounds) {
  ...
  async function getListInformation() {
    const listings = await fetch('/LS_API/getData')
      .then(data => data.json())
      .then((listings) => {
    ...
    for (let i = 0; i < listings.length; i++) {
      const { location } = listings[i].rental_property_information;
  ...
  function createMarker(googleMap, coordinates, imgSource, marker_selected) { // Construct a marker using createHTMLMapMarker
    ...
    const adjustedImgSource = imgSource;
    const _html = (marker_selected === true) ? `<img id="marker_selected" class="bounce" src="${adjustedImgSource}">` : `<img id="marker_default" class="bounce" src="${adjustedImgSource}">`;
