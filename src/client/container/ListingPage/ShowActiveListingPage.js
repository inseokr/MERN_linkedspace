...
function getListingContents(currentUser, listingDB, listing_prefix, type, child_listings, listingControl) {
    ...
    if (listing_prefix === '_3rdparty') {
      return (
        <a href={listing.url} target="_blank">
          <img className="img-responsive center" style={{ maxHeight: '80%', maxWidth: '100%', marginTop: '10px' }} src={FILE_SERVER_URL+listing.picture} alt="listingPicture" />
        </a>
      );
    }
    return (
  ...
  for (let index = 0; index < listingDB.length; index++) {
    const additionalClasses = (checkIfListingRead(currentUser, 'tenant', listingDB[index].id)===true)? "":'bottom_highlight';
